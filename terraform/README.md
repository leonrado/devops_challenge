...
deploy an Auto Scaling Group (ASG) and a MySQL DB across three environments (dev, prod) and two AWS accounts
...
* **Account**: At the top level are each of your AWS accounts, such as `dev-account`, `prod-account`, `mgmt-account`,
...
* **Environment**: Within each region, there will be one or more "environments", such as `dev`, etc. Typically,
